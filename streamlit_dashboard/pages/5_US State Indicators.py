...
# State Data IDs
states_data_id = {
    "Alabama": {"ur_id": "ALUR", "labor_id": "LBSSA01"},
    "Alaska": {"ur_id": "AKUR", "labor_id": "LBSSA02"},
    "Arizona": {"ur_id": "AZUR", "labor_id": "LBSSA04"},
    "Arkansas": {"ur_id": "ARUR", "labor_id": "LBSSA05"},
    "California": {"ur_id": "CAUR", "labor_id": "LBSSA06"},
    "Colorado": {"ur_id": "COUR", "labor_id": "LBSSA08"},
    "Connecticut": {"ur_id": "CTUR", "labor_id": "LBSSA09"},
    "Delaware": {"ur_id": "DEUR", "labor_id": "LBSSA10"},
    "District of Columbia": {"ur_id": "DCUR", "labor_id": "LBSSA11"},
    "Florida": {"ur_id": "FLUR", "labor_id": "LBSSA12"},
    "Georgia": {"ur_id": "GAUR", "labor_id": "LBSSA13"},
    "Hawaii": {"ur_id": "HIUR", "labor_id": "LBSSA15"},
    "Idaho": {"ur_id": "IDUR", "labor_id": "LBSSA16"},
    "Illinois": {"ur_id": "ILUR", "labor_id": "LBSSA17"},
    "Indiana": {"ur_id": "INUR", "labor_id": "LBSSA18"},
    "Iowa": {"ur_id": "IAUR", "labor_id": "LBSSA19"},
    "Kansas": {"ur_id": "KSUR", "labor_id": "LBSSA20"},
    "Kentucky": {"ur_id": "KYURN", "labor_id": "LBSSA21"},
    "Louisiana": {"ur_id": "LAUR", "labor_id": "LBSSA22"},
    "Maine": {"ur_id": "MEUR", "labor_id": "LBSSA23"},
    "Maryland": {"ur_id": "MDUR", "labor_id": "LBSSA24"},
    "Massachusetts": {"ur_id": "MAUR", "labor_id": "LBSSA25"},
    "Michigan": {"ur_id": "MIUR", "labor_id": "LBSSA26"},
    "Minnesota": {"ur_id": "MNUR", "labor_id": "LBSSA27"},
    "Mississippi": {"ur_id": "MSUR", "labor_id": "LBSSA28"},
    "Missouri": {"ur_id": "MTUR", "labor_id": "LBSSA29"},
    "Montana": {"ur_id": "MTUR", "labor_id": "LBSSA30"},
    "Nebraska": {"ur_id": "NEUR", "labor_id": "LBSSA31"},
    "Nevada": {"ur_id": "NVUR", "labor_id": "LBSSA32"},
    "New Hampshire": {"ur_id": "NHUR", "labor_id": "LBSSA33"},
    "New Jersey": {"ur_id": "NJURN", "labor_id": "LBSSA34"},
    "New Mexico": {"ur_id": "NMUR", "labor_id": "LBSSA35"},
    "New York": {"ur_id": "NYUR", "labor_id": "LBSSA36"},
    "North Carolina": {"ur_id": "NCUR", "labor_id": "LBSSA37"},
    "North Dakota": {"ur_id": "NDUR", "labor_id": "LBSSA38"},
    "Ohio": {"ur_id": "OHUR", "labor_id": "LBSSA39"},
    "Oklahoma": {"ur_id": "OKUR", "labor_id": "LBSSA40"},
    "Oregon": {"ur_id": "ORUR", "labor_id": "LBSSA41"},
    "Pennsylvania": {"ur_id": "PAUR", "labor_id": "LBSSA42"},
    "Puerto Rico": {"ur_id": "PRUR", "labor_id": "LBSSA43"},
    "Rhode Island": {"ur_id": "RIUR", "labor_id": "LBSSA44"},
    "South Carolina": {"ur_id": "SCUR", "labor_id": "LBSSA45"},
    "South Dakota": {"ur_id": "SDUR", "labor_id": "LBSSA46"},
    "Tennessee": {"ur_id": "TNUR", "labor_id": "LBSSA47"},
    "Texas": {"ur_id": "TXUR", "labor_id": "LBSSA48"},
    "Utah": {"ur_id": "UTUR", "labor_id": "LBSSA49"},
    "Vermont": {"ur_id": "VTUR", "labor_id": "LBSSA50"},
    "Washington": {"ur_id": "WAUR", "labor_id": "LBSSA54"},
    "West Virginia": {"ur_id": "WVUR", "labor_id": "LBSSA53"},
    "Wisconsin": {"ur_id": "WIUR", "labor_id": "LBSSA55"},
    "Wyoming": {"ur_id": "WYUR", "labor_id": "LBSSA56"}
}

line_colors = {
    "unemployment": "#032649",  # Dark blue
    "labor_force": "#EB8928",  # Orange
...
def download_csv(state_name, data_type):
    ...
    try:
        data_id = data_ids.get("ur_id") if data_type == "unemployment" else data_ids.get("labor_id", None)
        
        if not data_id:
            st.warning(f"No {data_type} ID available for {state_name}. Returning NULL values.")
            return pd.DataFrame(columns=['DATE', 'Labor Force' if data_type == "labor" else "Unemployment"]).assign(DATE=pd.NaT, **{data_type: None})
        ...
        if csv_data.empty:
            st.warning(f"No {data_type} data available for {state_name}. Returning NULL values.")
            return pd.DataFrame(columns=['DATE', 'labor Force' if data_type == "labor" else "Unemployment"]).assign(DATE=pd.NaT, **{data_type: None})

        column_name = "Unemployment" if data_type == "unemployment" else "labor Force"
        ...
        st.error(f"Error fetching {data_type} data for {state_name}: {e}")
        return pd.DataFrame(columns=['DATE', 'Labor Force' if data_type == "labor" else "Unemployment"]).assign(DATE=pd.NaT, **{data_type: None})
...
def plot_unemployment_labor_chart(state_name):
    unemployment_data = download_csv(state_name, "unemployment")
    labor_data = download_csv(state_name, "labor")

    if unemployment_data is not None and labor_data is not None:
        unemployment_data = unemployment_data[unemployment_data['DATE'].dt.year >= 2000]
        labor_data = labor_data[labor_data['DATE'].dt.year >= 2000]

        merged_data = pd.merge(unemployment_data, labor_data, on='DATE')
        ...
        fig.add_trace(go.Scatter(x=merged_data['DATE'], y=merged_data['Labor Force'], mode='lines',line=dict(color=line_colors["Labor Force"]), name="Labor Force"))
        ...
        fig.add_annotation(
            x=last_row['DATE'], y=last_row['Labor Force'],
            text=f" {last_row['Labor Force']:.1f}"+"%", showarrow=True, arrowhead=1, ax=-40, ay=40
        )
...
def export_to_pptx(labor_fig, gdp_fig):
    prs = Presentation()
    ...
    title1.text = " "
    img1 = BytesIO()
    labor_fig.write_image(img1, format="png")
    img1.seek(0)
...
def get_state_indicators_layout():
    ...
    st.subheader(f"{state_name} - Unemployment & labor Force")
    labor_fig = plot_unemployment_labor_chart(state_name)

    st.subheader(f"{state_name} - GDP Over Time")
    gdp_fig = plot_gdp_chart(state_name)

    if st.button("Export Charts to PowerPoint") and labor_fig is not None and gdp_fig is not None:
        pptx_file = export_to_pptx(labor_fig, gdp_fig)
        st.download_button(
