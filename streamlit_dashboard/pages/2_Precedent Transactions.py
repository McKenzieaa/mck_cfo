...
if selected_industries and selected_locations:
    ...
    if not selected_data.empty:
        ...
        fig1.update_layout(yaxis_title="EV/Revenue", xaxis_title=" ",bargap=0.4,bargroupgap=0.4,yaxis=dict(showgrid=False))
        fig1.add_shape(type="line",x0=-0.5, x1=len(avg_data['Target']) - 0.5,  y0=median_ev_revenue, y1=median_ev_revenue,line=dict(color="#EB8928", width=2, dash="dash"),  xref="x", yref="y")
        fig1.add_annotation(x=len(avg_data['Target']) - 1, y=median_ev_revenue + 0.2, text=f"Median: {median_ev_revenue:.1f}x",showarrow=False, font=dict(size=10, color="gray"), xanchor="left")
        ...
        fig2.update_layout(yaxis_title="EV/EBITDA", xaxis_title=" ",bargap=0.4,bargroupgap=0.4,yaxis=dict(showgrid=False))
        fig2.add_shape(type="line",x0=-0.5, x1=len(avg_data['Target']) - 0.5,  y0=median_ev_ebitda, y1=median_ev_ebitda,line=dict(color="#EB8928", width=2, dash="dot"),  xref="x", yref="y")
        fig2.add_annotation(x=len(avg_data['Target']) - 1, y=median_ev_ebitda + 0.2, text=f"Median: {median_ev_ebitda:.1f}x",showarrow=False, font=dict(size=10, color="gray"), xanchor="left")
